...
function AuthLoadingScreen() {
  return (
    <main className="grid min-h-screen place-items-center bg-[#161612] text-white">
...
function LoginGate({ password, busy, message, onPasswordChange, onSubmit }: { password: string; busy: boolean; message?: string; onPasswordChange: (value: string) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return (
    <main className="relative grid min-h-screen overflow-hidden bg-[#161612] px-4 py-8 text-white">
      ...
      <section className="relative m-auto w-full max-w-md rounded-md border border-white/12 bg-[#161612]/90 p-6 shadow-[0_24px_70px_rgba(0,0,0,0.28)] backdrop-blur-xl sm:p-8">
        <div className="mb-6">
        ...
        <form className="grid gap-4" onSubmit={onSubmit}>
          <div className="grid gap-2">
            ...
            <Input id="dashboard-password" type="password" autoComplete="current-password" value={password} onChange={(event) => onPasswordChange(event.target.value)} className="h-12 rounded-md border-white/15 bg-[#161612]/75 text-white placeholder:text-white/40" placeholder="Enter password" autoFocus />
          </div>
          {message ? <Alert variant="destructive" className="rounded-md px-3 py-2 text-sm font-semibold">{message}</Alert> : null}
          <Button type="submit" disabled={busy} className="h-12 rounded-md bg-[#161612] font-black text-white hover:bg-[#1f1f1f]">
