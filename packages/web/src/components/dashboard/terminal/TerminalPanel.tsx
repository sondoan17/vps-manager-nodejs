...
export function TerminalPanel({ vps, enabled = true }: { vps: VpsRecord; enabled?: boolean }) {
  ...
  return <div className="min-w-0 space-y-3">
    ...
    <div aria-live="polite" className="flex flex-wrap justify-between gap-2 font-mono text-xs text-white/40"><span>{labels[status]} · Ctrl+C copies selection · Copy Ctrl+Shift+C / ⌘C · Paste Ctrl+Shift+V / ⌘V</span><span>{expiresAt ? `Expires ${new Date(expiresAt).toLocaleTimeString()}` : "SSH transport · Monitoring status is shown in the dashboard header"}</span></div>
