...
type FormState = Record<"displayName" | "host" | "port" | "username" | "provider" | "region" | "notes", string>;
...
export function EditServerDialog({ vps, open, onOpenChange, onSave }: {
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    try {
      await onSave(vps, { displayName, host, port, username, provider: form.provider.trim(), region: form.region.trim(), notes: form.notes.trim() });
      onOpenChange(false);
    } catch (cause) {
      ...
      <AlertDialogContent className="max-h-[90vh] w-[calc(100%-2rem)] max-w-2xl overflow-y-auto border-white/10 bg-[#111318] p-0 text-white">
        <form onSubmit={submit}>
          <AlertDialogHeader className="border-b border-white/10 px-5 py-5 sm:px-6">
            ...
            <AlertDialogDescription className="text-white/55">Update the name, SSH connection, and location details.</AlertDialogDescription>
          </AlertDialogHeader>
          {form ? <div className="grid gap-4 px-5 py-5 sm:grid-cols-2 sm:px-6">
            ...
            <Label className="text-white sm:col-span-2">Username<Input required maxLength={120} autoComplete="username" value={form.username} onChange={(e) => field("username", e.target.value)} /></Label>
            <Label className="text-white">Provider<Input maxLength={120} value={form.provider} onChange={(e) => field("provider", e.target.value)} /></Label>
            <Label className="text-white">Region<Input maxLength={120} value={form.region} onChange={(e) => field("region", e.target.value)} /></Label>
            <Label className="text-white sm:col-span-2">Notes<textarea className="mt-1.5 min-h-24 w-full resize-y border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-white outline-none transition focus:border-accent focus:ring-4 focus:ring-accent/20" maxLength={2000} value={form.notes} onChange={(e) => field("notes", e.target.value)} /></Label>
