...
function ContainerRow({ container, name, stale, roomy = false }: { container: DashboardDockerContainerMetric; name: string; stale: boolean; roomy?: boolean }) {
  ...
  return <li className={`grid min-w-0 grid-cols-[minmax(0,1fr)_3.75rem_4.5rem] items-center gap-2 border-t px-2 ${roomy ? "py-3 text-xs sm:grid-cols-[minmax(0,1fr)_6rem_7rem] sm:px-4 sm:text-sm" : "py-2 text-[11px]"} ${running ? "border-white/[0.06] text-white/55" : "border-amber-400/20 bg-amber-400/[0.07] text-white/75"}`} title={`${container.name} · ${container.image} · ${container.status ?? container.state}`}>
...
export function DockerMetricsPanel({ vps, dockerMetrics, busy, onToggle, presentation = "compact" }: { vps: VpsRecord; dockerMetrics?: DashboardOverview["dockerMetrics"][number]; busy: boolean; onToggle: (vps: VpsRecord) => void; presentation?: "compact" | "detail" }) {
  ...
  if (!enabled) body = <div className="rounded-sm border border-white/[0.06] bg-black/10 p-3"><p className="text-xs font-medium text-white/70">Docker monitoring is off.</p><p className="mt-1 text-[11px] text-white/45">Enable it to see container health and resource usage.</p></div>;
    ...
    {containers.length ? <><div className="grid grid-cols-[minmax(0,1fr)_3.75rem_4.5rem] gap-2 px-2 text-[9px] uppercase tracking-[0.1em] text-white/35"><span>Container</span><span className="text-right">CPU</span><span className="text-right">Memory</span></div><ul>{containers.slice(0, 3).map((container) => <ContainerRow key={container.containerKey} container={container} name={names.get(container.containerKey) ?? container.name} stale={notLive} />)}</ul></> : <p className="text-xs text-white/50">No containers reported.</p>}
    <Link to={`/vps/${encodeURIComponent(vps.id)}/docker`} className="inline-flex min-h-8 items-center text-[11px] font-medium text-sky-200 underline-offset-4 hover:text-white hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/60" aria-label={`View Docker details for ${vpsDisplayName(vps)}`}>View Docker details →</Link>
  </div>;
    ...
    {visible.length ? <><div className={`grid grid-cols-[minmax(0,1fr)_3.75rem_4.5rem] gap-2 px-2 text-[9px] uppercase tracking-[0.1em] text-white/35 ${detail ? "sm:grid-cols-[minmax(0,1fr)_6rem_7rem] sm:px-4" : ""}`}><span>Container</span><span className="text-right">CPU</span><span className="text-right">Memory</span></div><ul>{visible.map((container) => <ContainerRow key={container.containerKey} container={container} name={names.get(container.containerKey) ?? container.name} stale={notLive} roomy={detail} />)}</ul></> : <p className="py-3 text-center text-xs text-white/50">No containers match this view.</p>}
    {filtered.length > detailLimit || expanded ? <button type="button" className="text-[11px] font-medium text-sky-200 underline-offset-4 hover:text-white hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/60" aria-expanded={expanded} onClick={() => setExpanded((value) => !value)}>{expanded ? "Show less ↑" : `Show more (max 20) →`}</button> : null}</> : <p className="text-xs text-white/50">No containers reported.</p>}
  </div>;

  return <div className={`${detail ? "border border-white/10 bg-gradient-to-br from-white/[0.055] to-white/[0.015] p-4 sm:p-5" : "border-0 bg-white/[0.03] px-3 py-2"} rounded-none shadow-none`}><div className={`${detail ? "mb-4 border-b border-white/10 pb-4" : "mb-2"} flex items-center justify-between gap-3`}><div><span className={`${detail ? "text-sm text-white" : "text-[11px] uppercase tracking-[0.14em] text-white/60"} font-medium`}>{detail ? "Container snapshot" : "Docker"}</span>{detail ? <p className="mt-1 text-xs text-white/40">Monitoring is {enabled ? "enabled" : "disabled"} for this server.</p> : null}</div><Button type="button" size="sm" variant={enabled ? "secondary" : "outline"} className={`${detail ? "min-w-20" : "h-7 px-2 text-[11px]"} rounded-none`} disabled={busy} aria-pressed={enabled} aria-label={`${enabled ? "Disable" : "Enable"} Docker metrics for ${vpsDisplayName(vps)}`} onClick={() => enabled ? onToggle(vps) : setDialogOpen(true)}>{busy ? "Updating…" : enabled ? "On" : "Off"}</Button></div>{body}
