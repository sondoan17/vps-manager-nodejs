...
export function DockerMetricsPanel({ vps, dockerMetrics, busy, onToggle, presentation = "compact" }: { vps: VpsRecord; dockerMetrics?: DashboardOverview["dockerMetrics"][number]; busy: boolean; onToggle: (vps: VpsRecord) => void; presentation?: "compact" | "detail" }) {
  ...
  if (!enabled) body = <div className="rounded-sm border border-white/[0.06] bg-black/10 p-3"><p className="text-xs font-medium text-white/70">Docker monitoring is off.</p><p className="mt-1 text-[11px] text-white/45">Enable it to see container health and resource usage.</p></div>;
    ...
    {containers.length ? <><div className="grid grid-cols-[minmax(0,1fr)_3.75rem_4.5rem] gap-2 px-2 text-[9px] uppercase tracking-[0.1em] text-white/35"><span>Container</span><span className="text-right">CPU</span><span className="text-right">Memory</span></div><ul>{containers.slice(0, 3).map((container) => <ContainerRow key={container.id} container={container} name={names.get(container.id) ?? container.name} stale={notLive} />)}</ul></> : <p className="text-xs text-white/50">No containers reported.</p>}
    ...
    {visible.length ? <><div className={`grid grid-cols-[minmax(0,1fr)_3.75rem_4.5rem] gap-2 px-2 text-[9px] uppercase tracking-[0.1em] text-white/35 ${detail ? "sm:grid-cols-[minmax(0,1fr)_6rem_7rem] sm:px-4" : ""}`}><span>Container</span><span className="text-right">CPU</span><span className="text-right">Memory</span></div><ul>{visible.map((container) => <ContainerRow key={container.id} container={container} name={names.get(container.id) ?? container.name} stale={notLive} roomy={detail} />)}</ul></> : <p className="py-3 text-center text-xs text-white/50">No containers match this view.</p>}
