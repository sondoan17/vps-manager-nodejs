...
export function agentJobFor(vps: VpsRecord, jobs: DashboardJob[]) {
  const lifecycleJobs = jobs.filter(
    (job) =>
      job.vpsId === vps.id &&
      (job.type === "install-agent" || job.type === "uninstall-agent"),
  );
...
export function AgentLifecycleStatus({ vps, jobs, compact = false }: {
  ...
  if (active) {
    ...
    const label = stepLabels[job.step || ""] || (removing ? "Removing agent" : "Installing agent");
    return (
      <div className={compact ? "min-w-[150px]" : "mt-3 border-t border-white/10 pt-3"} aria-live="polite">
        ...
        <div role="progressbar" aria-label={`${removing ? "Agent removal" : "Agent install"}: ${label}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress} className="mt-2 h-1.5 overflow-hidden bg-white/10">
