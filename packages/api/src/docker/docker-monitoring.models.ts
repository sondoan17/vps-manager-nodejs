...
export type DockerMetricRollup = { id: string; vpsId: string; agentInstanceId: string; scope: DockerScope; metricName?: string; containerKey?: string; cohortDigest?: string; bucketStart: string; formulaVersion: number; firstAt: string; lastAt: string; sampleCount: number; gaugeMin?: number; gaugeMax?: number; gaugeSum?: number; gaugeAverage?: number; counterFirst?: number; counterLast?: number; counterIncrease?: number; resetCount: number; expectedSamples: number; observedSamples: number; partialSampleCount: number; gapCount: number; coverageRatio: number };
