...
export function parseTerminalMessage(
  ...
  if (Buffer.isBuffer(raw) && !Buffer.from(text, "utf8").equals(raw))
    throw new Error("PROTOCOL_ERROR");
  if (Buffer.byteLength(text) > MAX_TERMINAL_FRAME_BYTES)
    throw new Error("PROTOCOL_ERROR");
  let value: unknown;
  try {
  ...
  if (!value || typeof value !== "object" || !codes.has((value as any).type))
    throw new Error("PROTOCOL_ERROR");
  ...
  if (message.type === "open") {
    if (
      message.version !== 1 ||
      typeof message.vpsId !== "string" ||
      !message.vpsId.trim() ||
      message.vpsId !== message.vpsId.trim() ||
      message.vpsId.length > 200 ||
      !validPty(message.cols, message.rows)
    )
      throw new Error("PROTOCOL_ERROR");
    return {
      type: "open",
  ...
  if (message.type === "input") {
    if (
      ...
      Buffer.byteLength(message.data) > MAX_TERMINAL_INPUT_BYTES
    )
      throw new Error("PROTOCOL_ERROR");
    return { type: "input", data: message.data };
  }
  if (message.type === "resize") {
    if (!validPty(message.cols, message.rows))
      throw new Error("PROTOCOL_ERROR");
...
export function validPty(cols: unknown, rows: unknown): boolean {
  return (
    boundedInt(cols, PTY_LIMITS.minCols, PTY_LIMITS.maxCols) &&
    boundedInt(rows, PTY_LIMITS.minRows, PTY_LIMITS.maxRows)
  );
}

export function encodeTerminalMessage(message: TerminalServerMessage): string {
