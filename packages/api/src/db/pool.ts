...
export function createDatabasePool(config: AppConfig): Pool {
  if (config.storageDriver !== "postgres") {
    throw new Error("Cannot create a database pool unless STORAGE_DRIVER=postgres");
  }
...
export async function withTransaction<T>(pool: DatabasePool, run: (client: PoolClient) => Promise<T>): Promise<T> {
