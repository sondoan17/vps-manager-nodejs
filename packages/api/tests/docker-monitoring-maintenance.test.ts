...
describe("docker monitoring JSON maintenance (I3 slice)", () => {
  ...
  it("records lastMaintenanceAt on the first successful run", async () => {
    const r = repo();
    await r.pruneSamplesEventsAndStorage({
  ...
  it("preserves legacy metadata normalization and updates the timestamp on each run", async () => {
    ...
    expect(
      new Date(second.lastMaintenanceAt!).getTime(),
    ).toBeGreaterThanOrEqual(new Date(first.lastMaintenanceAt!).getTime());
  });
  ...
  it("prunes samples/events strictly before cutoff and preserves latest state", async () => {
    ...
    expect(
      (await r.listHostSamples({ vpsId: "vps-a" })).data.map((s) => s.id),
    ).toEqual(["new"]);
    expect(
      (
        await r.listContainerSamples({
          vpsId: "vps-a",
          agentInstanceId: "inst1",
          containerKey: "ck1",
        })
    ...
    expect(
      (await r.listEvents({ vpsId: "vps-a" })).data.map((e) => e.id),
    ).toEqual(["e-new"]);
    expect((await r.getStorageLatest("vps-a"))?.snapshotId).toBe(
      "snap-storage",
    );
  });
  ...
  it("enforces per-VPS newest-first caps after cutoff filtering", async () => {
    const r = repo();
    const at = (n: number) =>
      `2026-03-${String(n).padStart(2, "0")}T00:00:00.000Z`;
    await seedDockerMonitoringForTests(file, {
      samples: [
    ...
    expect(
      (await r.listHostSamples({ vpsId: "vps-b" })).data.map((s) => s.id),
    ).toEqual(["b2", "b1"]);
    expect(
      (await r.listEvents({ vpsId: "vps-a" })).data.map((e) => e.id),
    ).toEqual(["ea3"]);
  ...
  it("rejects invalid maintenance options without mutating the store", async () => {
    ...
    await expect(
      r.pruneSamplesEventsAndStorage({
        cutoff: "not-a-date",
        samplesPerVps: 10,
        eventsPerVps: 10,
      }),
  ...
  it("leaves the file byte-for-byte unchanged when maintenance exceeds its rewrite limit", async () => {
    ...
    // Arrange / Act / Assert
    await expect(
      limited.pruneSamplesEventsAndStorage({
        cutoff: "2026-01-01T00:00:00.000Z",
        samplesPerVps: 10,
        eventsPerVps: 10,
      }),
    ).rejects.toThrow(/rewrite exceeds/i);
