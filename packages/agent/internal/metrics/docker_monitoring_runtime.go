...
type DockerFinalizeFunc func(*DockerMetricsV2)
...
// Docker event/storage calls use the existing v1 client and base URL.
func (c *Collector) collectDockerAPI(ctx context.Context, out *DockerMetricsV2, starts ...time.Time) {
				...
				if resp, err := client.Do(req); err == nil {
					func() {
						...
						if resp.StatusCode >= 200 && resp.StatusCode < 300 {
							evs, win, prop, err := collectDockerEventWindow(ctx, resp.Body, in.SinceNano, in.UntilNano, in.AgentInstanceID, in.FromDigests, c.v2KeyFunc())
							if err == nil {
...
// dockerFromV1 derives a minimal canonical v2 snapshot from the same
...
func dockerFromV1(v1 *DockerMetrics, keyForID DockerContainerKeyFunc, rotations ...uint64) (out *DockerMetricsV2) {
	if v1 == nil {
		return nil
	}
	if keyForID == nil {
	...
	sel := make([]DockerSelection, 0, len(v1.Containers))
	...
	for _, vc := range v1.Containers {
		...
		byKey[key] = sanitizeDockerDisplay(DockerContainerV2{
			ContainerKey:     key,
			Name:             vc.Name,
			Image:            vc.Image,
			State:            vc.State,
	...
	agg := dockerSampledAggregate(out.Containers, v1.ContainerTotal)
...
// collectDocker builds the additive snapshot from an already-collected v1
...
func (c *Collector) collectDockerSnapshot(ctx context.Context, v1 *DockerMetrics, starts ...time.Time) (out *DockerMetricsV2) {
	if v1 == nil {
		return nil
	}
	c.dockerMu.RLock()
	...
	c.dockerRotation++
	c.dockerMu.Unlock()
	out = dockerFromV1(v1, keyFunc, rotation)
	if out == nil {
		return nil
	}
