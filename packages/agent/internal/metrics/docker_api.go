...
// the Linux wrapper owns Unix socket dialing, socket existence checks, base
// URL validation, and the overall collection deadline, then delegates here.
// Never returns nil so the caller can always attach the result.
//
// Read-only allowlist (GET only): /version, /containers/json, /containers/<id>/stats.
// Engine version collection is bounded and best-effort: failures never fail
// the overall collection, the whitelisted fields are simply left empty.
//
// Concurrency note (Phase 3): per-container stats are fetched sequentially for
...
func collectDockerFromAPI(ctx context.Context, client *http.Client, baseURL string) *DockerMetrics {
	...
	// while /containers/json and per-container stats continue normally.
	var ver *dockerEngineVersion
	func() {
		versionCtx, cancel := context.WithTimeout(ctx, DockerVersionBudget)
		defer cancel()
		ver = getEngineVersion(versionCtx, client, baseURL)
	}()

	// 1. List all containers.
	containers, err := listContainers(ctx, client, baseURL)
	if err != nil {
	...
	if ver != nil {
		result.EngineVersion = ver.EngineVersion
		result.APIVersion = ver.APIVersion
		result.OS = ver.OS
		result.Architecture = ver.Architecture
	}
	...
	for _, raw := range containers {
		...
		if raw.State == "running" {
			...
			stats, err := getContainerStats(ctx, client, baseURL, raw.ID)
			if err != nil {
...
func getEngineVersion(ctx context.Context, client *http.Client, baseURL string) *dockerEngineVersion {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, dockerAPIURL(baseURL, "/version"), nil)
	if err != nil {
		return nil
	}
	...
	var raw dockerEngineVersion
	if err := json.NewDecoder(io.LimitReader(resp.Body, 16*1024)).Decode(&raw); err != nil {
		return nil
	}
	raw.EngineVersion = cappedString(raw.EngineVersion, MaxEngineVersionLen)
	raw.APIVersion = cappedString(raw.APIVersion, MaxAPIVersionLen)
	raw.OS = cappedString(raw.OS, MaxDockerOSLen)
	raw.Architecture = cappedString(raw.Architecture, MaxDockerArchLen)
	return &raw
}
...
// ---------------------------------------------------------------------------
// Docker API calls (GET-only allowlist)
// ---------------------------------------------------------------------------
